...
pub const CodexExecutor = struct {
    ...
    fn executeErased(ptr: *anyopaque, allocator: std.mem.Allocator, req: types.Request, _: types.Options) anyerror!types.Response {
        const self: *CodexExecutor = @ptrCast(@alignCast(ptr));
...
test "codex executor has correct defaults" {
    var exec = CodexExecutor.init("test-key");
    try std.testing.expectEqualStrings("codex", exec.executor().providerName());
    try std.testing.expectEqualStrings("https://api.openai.com", exec.base_url);
}
